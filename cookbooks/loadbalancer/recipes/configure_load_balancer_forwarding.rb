...
## General Variables
doc_root=/home/webapps/#{node[:loadbalancer][:lb_application]}/current
deploy_dir=/home/webapps/#{node[:loadbalancer][:lb_application]}/releases

mkdir -p $deploy_dir
mkdir -p $log_dir
ln -nfs $deploy_dir $doc_root

apache_maint_page="#{node[:loadbalancer][:lb_maintenance_page]}"
...
# Locally(Off)
options="-s #{node[:loadbalancer][:lb_website_dns]} -d $doc_root -l $log_dir $install_dir_option -m "$apache_maint_page" -k on -f off"
...
if [ "#{lb_bind_port}" == "443" ]; then
  ...
  echo "#{node[:loadbalancer][:lb_ssl_key]}" > $key_dir/#{node[:loadbalancer][:lb_website_dns]}.key
  echo "#{node[:loadbalancer][:lb_ssl_certificate]}" > $key_dir/#{node[:loadbalancer][:lb_website_dns]}.crt
  #if [ -n "$OPT_SSL_CERTIFICATE_CHAIN" ]; then
  #  echo "Installing SSL certificate chain"
  #  echo "$OPT_SSL_CERTIFICATE_CHAIN" > $key_dir/#{node[:loadbalancer][:lb_website_dns]}.sf_crt
  #fi
  chmod 400 $key_dir/*
