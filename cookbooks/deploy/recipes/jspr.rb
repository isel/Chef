...
bash 'Deploying prios' do
  code <<-EOF
    ...
    sed -i "s@/JSPR/ver1/@/JSPR/ver#{FileList["#{node['deploy_scripts_dir']}/JSPR/JSPR/ver*"][0].split('ver')[1]}/@" /var/www/Prios/Tests/jstestload.html
  EOF
...
template '/var/www/Prios/Tests/settings.js' do
  source 'compass_settings.erb'
  variables(
    :host => "#{ node[:deploy][:domain].nil? ? node[:ipaddress] : node[:deploy][:domain] }/Prios/Tests"
  )
end
